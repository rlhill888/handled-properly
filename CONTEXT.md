...
**About Content**:
The `site_about_content` table — a singleton row (exactly one, not a list) still holding a Content Block array. Currently unused: the public `/about` page is hand-coded (see [`0031-about-page-hand-coded-not-block-driven`](./docs/adr/0031-about-page-hand-coded-not-block-driven.md)) and no longer reads it, though the admin's `/portal/admin/website/about` editor for it still exists and still works — it just has no effect on the live page right now.
