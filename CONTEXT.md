...
Admin-managed content shown on the public marketing site (`/`, `/about`, `/about-and-connect`, `/events`, `/contact`), edited from `/portal/admin/website`. Read by public pages via the service-role client, not a per-session one — see [`0017-blog-posts-are-not-linked-to-events`](./docs/adr/0017-blog-posts-are-not-linked-to-events.md).
...
**About Content**:
The `site_about_content` table — a singleton row (exactly one, not a list) holding the public `/about-and-connect` page's editable headshot photo (`headshot_path`) and about-section text (`about_body`, plain text). Not read by `/about` — that's a different page with its own separate table, About Page Content below. `/about-and-connect` is bespoke JSX, not Content-Block-driven, but reads this row directly, along with Social Link and Featured Item below — see [`0032-about-page-editable-landing-page`](./docs/adr/0032-about-page-editable-landing-page.md) and [`0033-about-and-connect-is-a-separate-page`](./docs/adr/0033-about-and-connect-is-a-separate-page.md).
_Avoid_: About Page Content (that's `/about`'s own singleton row — a different table, a different page, a different shape of content)

**Social Link**:
An admin-managed icon button shown on `/about-and-connect`, opening an external URL in a new tab. Icon is admin-uploaded (any platform, not a fixed set), same reasoning as Trusted Partner's logo.
_Avoid_: Contact (Social Link has no relationship to any person/company record the system tracks — it's just an icon and a URL)

**Featured Item**:
An admin-managed thing shown on `/about-and-connect` that the admin sells or wants to promote — a title, description, optional image, and optional outbound link. Not a Blog Post (no date/category/slug, and About & Connect isn't `/events`) and not a Trusted Partner (has a description and a link; Trusted Partner has neither).

**About Page Content**:
The `site_about_page` table — a singleton row holding the text and one image for each of `/about`'s seven named sections (Hero/Introduction, Our Story, Who We Are, What We Do, Our Mission, Our Vision, Our Values) plus its closing CTA: `headline`/`hero_intro`/`hero_tagline`/`hero_image_path`, `story_body`/`story_image_path`, `who_we_are_body`/`who_we_are_image_path`, `what_we_do_body`/`what_we_do_image_path`, `mission_body`/`mission_image_path`, `vision_body`/`vision_image_path`, `values_image_path` (the Values section's one banner photo — the individual cards are Value, below), and `cta_heading`/`cta_button_text`. Body fields with multiple paragraphs use a blank line as the separator; `headline` uses one line break per line (last line always shown muted). Hero and Our Story fall back to the bundled `/about/hero.png`/`/about/story.png` placeholders when unset; every other image field just renders without an image, or (Our Mission/Our Vision) a plain placeholder box, until the admin uploads one. `/about` stays bespoke, hand-coded JSX (see [`0031`](./docs/adr/0031-about-page-hand-coded-not-block-driven.md)) — this table makes its specific text/images admin-editable without moving it onto Content Blocks or onto About Content's shape. See [`0034-about-page-editable-fields-not-blocks`](./docs/adr/0034-about-page-editable-fields-not-blocks.md) and [`0035-about-page-seven-sections`](./docs/adr/0035-about-page-seven-sections.md).
_Avoid_: About Content (that's `/about-and-connect`'s singleton row, a different table/page)

**Value**:
One "Our Values" card on `/about` — a title and short explanation, admin-managed (add/edit/remove/reorder) like Social Link and Featured Item rather than a fixed count (the content spec calls for 3–6). Stored in `site_about_values`, ordered by `sort_order`; the card's displayed number (01, 02, …) is computed from position at render time, not stored. Renamed from "Differentiator" (a "what makes us different" grid) once `/about` grew a dedicated Values section — see [`0035-about-page-seven-sections`](./docs/adr/0035-about-page-seven-sections.md).
_Avoid_: Featured Item (Value has no image or link, and lives on `/about`, not `/about-and-connect`)

**Content Block**:
One reusable, admin-stacked section of a page's content — a Blog Post's body (About Content used to be block-composed too, and briefly again; it isn't anymore, see [`0032-about-page-editable-landing-page`](./docs/adr/0032-about-page-editable-landing-page.md) and [`0033`](./docs/adr/0033-about-and-connect-is-a-separate-page.md)). Eight types: `title` (plain text heading — optionally a second muted-gray line, and optionally a short side-by-side supporting paragraph with a divider between them), `paragraph` (rich text, italic only), `image` (a single image with an optional caption and an optional grayscale filter), `image_text` (image + rich text side by side, bold/italic toolbar, optionally styled as a dark "statement" banner), `gallery` (a grid of images with titles/captions, or a `carousel` display mode with arrows/dots and optional pausable autoplay), `divider` (a plain horizontal rule with no fields), `features` (a numbered grid of short title+description items, numbers computed at render time from position), and `cta` (a full-width heading + single button banner, light or dark). Stored as an ordered JSON array (`blocks`), edited via the admin's `BlockEditor` (add/delete, reorder by dragging a handle or by Move Up/Down buttons) and, for rich-text blocks, a `RichTextEditor` whose toolbar varies by block type. See [`0018-four-content-block-types`](./docs/adr/0018-four-content-block-types.md) for why the type list started small, [`0019-sanitize-block-html-server-side`](./docs/adr/0019-sanitize-block-html-server-side.md) for why block HTML is sanitized before storage, [`0023-title-and-paragraph-replace-text`](./docs/adr/0023-title-and-paragraph-replace-text.md) for why Title and Paragraph are separate types rather than one generic Text block, [`0024-remove-highlight-and-narrow-image-text-toolbar`](./docs/adr/0024-remove-highlight-and-narrow-image-text-toolbar.md) for why there's no highlight anywhere and Image & Text has no headings, [`0025-drag-to-reorder-blocks`](./docs/adr/0025-drag-to-reorder-blocks.md) for why dragging was added back alongside the buttons rather than replacing them, and [`0030-heading-and-features-and-cta-blocks`](./docs/adr/0030-heading-and-features-and-cta-blocks.md) for Title's muted/supporting text and the Features/CTA additions.
